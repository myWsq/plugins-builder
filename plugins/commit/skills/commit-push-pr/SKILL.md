---
name: commit-push-pr
description: Delegate the full commit → push → pull-request flow to a cheap subagent. Use when the user asks to commit and open a PR, ship changes as a pull request, or says things like "commit and open a PR", "提交并开 PR". The main model must not run git or gh itself; it delegates the whole flow to a subagent running on the host's cheapest available model and relays the PR URL.
---
