...
No pull request, no `git switch`, no `git checkout`, no new branch — this pushes whatever branch is already checked out, and the default branch is a legitimate target here. When the change belongs on a pull request instead, use git:pr.
