...
2. **Execution mode**: do not describe the modes in your own words — locate the installed `dev-execute-plan` skill by name, read its "Choose execution mode" section (the canonical definition), and build this item's options from it: the modes, the recommended default, and the external-CLI disclosure, plus model if the user cares. Answering here is final — `dev-execute-plan` will not re-ask, and choosing an external CLI is the informed consent its rules require. If `dev-execute-plan` is not installed or cannot be located, omit this item entirely: it alone asks about execution, at dispatch time.
