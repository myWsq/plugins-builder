...
3. **Autopilot**: confirm that after this answer `dev:write-plan` and `dev:execute-plan` run to completion without further confirmation — the plan is committed and executed automatically. Offer two explicit opt-ins beside it: a review pause (stop after the plan is written, for users who want to read the plan first), and an advisor review (`dev:write-plan` sends the finished plan to `dev:advisor`, revises it on the findings, then continues into execution without stopping). Offer the advisor review only when the latest injected `<dev-orchestrator>` block (in Claude Code the plugin's hooks inject one at session start and when this skill starts) does not say `top-tier: yes`: an orchestrator already on the advisor's tier gains nothing from it, so say in one clause that the option is omitted for that reason. Without a block, offer it. The answer sets the start contract's **Stop after** and **Plan review** values: autopilot means `implementation` with `none`, the review pause means `plan`, the advisor review means `implementation` with `advisor`.
