...
3. **Autopilot**: confirm that after this answer `dev:write-plan` and `dev:execute-plan` run to completion without further confirmation — the plan is committed and executed automatically. Three answers: a plan audit (`dev:write-plan` sends the finished plan to a top-tier subagent that reads the code itself, revises it on the findings, then continues into execution without stopping), plain autopilot (straight to execution), and a review pause (stop after the plan is written, for users who want to read the plan first). The plan audit is the **recommended default whenever it is available**: the host has a subagent tool, and the latest injected `<dev-orchestrator>` block (in Claude Code the plugin's hooks inject one at session start and when this skill starts) does not say `top-tier: yes`; without a block, treat the tier as unknown and still recommend it. When the block says `top-tier: yes`, an orchestrator already on the audit's tier gains nothing from it: omit the option, say in one clause why, and recommend plain autopilot. When the subagent tool is missing, omit it likewise and recommend plain autopilot. The answer sets the start contract's **Stop after** and **Plan review** values: autopilot means `implementation` with `none`, the review pause means `plan`, the plan audit means `implementation` with `audit`.
The workspace is deliberately not an item here. Where the work happens is a fact, not a decision: `dev:write-plan` reads the latest injected `<dev-workspace>` block (in Claude Code the plugin's hooks inject one at session start and when this skill starts) and acts on its `kind` — from the main worktree it cuts the branch and worktree named after the requirement and continues there, inside a linked worktree it stays put. Say which of the two will happen in one line next to the question, and name any pending changes from the block's `pending` count (or `git status --porcelain` when no block is present), since a dirty tree stops the move — as information, never as something to answer. A user who says "plan here" still overrides it.
