...
Exploration then closes with the **departure check** — the workflow's last confirmation gate, and a fixed one: whenever the chain continues into planning it asks a single structured question bundling the same four items, even for a small change where every item already has a recommended value.

1. **Direction** — the approved direction, restated in one or two sentences.
2. **Execution mode** — the question defined canonically in `dev:execute-plan`: the generic subagent on `opus`, a model-pinned executor (eligible vendors listed directly, unverified ones labeled, unavailable ones omitted), or self-execution. Answering here is final.
3. **Autopilot** — confirm that `dev:write-plan` and `dev:execute-plan` then run to completion without further confirmation, or take the **review pause** to stop and read the plan first.
4. **Workspace** — asked only from the repository's main worktree: the branch and worktree that will be cut, and any pending changes that would stop the move. Asking to stay is the opt-out.

The four answers become the **start contract** the three skills share — `Direction and scope`, `Stop after`, `Execution`, `Workspace` — carried through the handoff and recorded in the plan with each value's basis. Downstream skills treat them as standing authorization and never re-ask; a later explicit user instruction supersedes an older plan field. A specified executor that cannot run is reported as a blocker, never silently replaced.
...
| "Think through how this could work." | Discuss and stop; no departure check, no executor question. |
...
**Off the main worktree.** When the session is in the repository's main worktree — on any branch — `dev:write-plan` first cuts a branch `dev/YYYYMMDD-slug` and a worktree `.claude/worktrees/YYYYMMDD-slug` from the current `HEAD`, named after the requirement with the same id as the plan, and switches the session into it. The plan commit, the implementation commits, and the status updates all land on that branch; the main worktree keeps its branch untouched, and merging back is yours to request. A dirty tree stops the move rather than leaving uncommitted changes behind; the agent reports the pending changes. The planner installs nothing there — `dev:execute-plan` installs dependencies at preflight. Say "plan here" — in your request or at the departure check's workspace item — or start from an existing linked worktree, to skip the move.
...
The default subagent is the host's generic one using `opus` where supported, otherwise the host default. To run a non-Claude model served through your API relay, pick one at the departure check's execution item or name it in your request — the plugin ships one executor per relay vendor, exposed as `dev:<vendor>-executor`, each pinning a full model ID in its frontmatter. The agent files under `agents/` are the source of truth for which model each one runs — read the ID there rather than trusting any list in the docs, since relay model IDs move. You can define more the same way in `.claude/agents/` or the user configuration's `agents/` directory. Dispatch pinned executors without a `model` override. Verify which model actually served the run (for example via relay-side logs); a listed model is not proof of actual routing.
