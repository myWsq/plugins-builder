...
`dev` is a small collection of agent skills for plan-driven software development. It splits a development task into three explicit phases — exploration (the product behaviour first when the change is perceptible, then the code), implementation planning, and plan execution — and front-loads every decision that needs a human into the first phase. It asks only about unresolved user decisions and proceeds to the endpoint you requested, without a requirement-confirmation gate.
...
| --- | --- | --- |
| `dev:explore` | Read-only exploration: for a change its consumer can perceive, clarify the product first — interaction flow, states, UI structure, scope — from the product surface and summarize it; then map the relevant code, grill the technical design question by question until it holds up, compare approaches, and hand off within the requested scope. Can also stress-test an existing plan or design. | Product conclusions when the change is perceptible, a codebase map, resolved decisions, a settled direction, and any existing execution preferences. |
...
dev:explore ──> dev:write-plan ──> dev:execute-plan
 discussion       plan              implementation + verification
```

The request determines where the chain stops: discussion ends in chat, a plan-only request ends after writing the plan, and an implementation request continues through execution and verification. Readiness does not expand authorization. STOP and BLOCK conditions still halt the chain — those are safety stops, not confirmations — and pushing, opening PRs, or merging always require an explicit user request.

## How the flow works

### 1. Explore and grill (`dev:explore`)

`dev:explore` modifies nothing. It triages the request first, and for a change its consumer can perceive — a page, a flow, copy, a CLI command or its output, the shape of an API call — it **clarifies the product before reading the implementation**: it looks only at the product surface the consumer already sees, grills the consumer and trigger, product form, interaction flow, structure-level UI (layout, components, the empty/loading/error/success states, key copy), scope, and consumer-visible acceptance, then summarizes the conclusions without a confirmation question. Visual design stays with the project's design system. Internal changes — bug fixes, refactors, infrastructure, performance — skip this stage.

Then it reads the relevant code, validation commands, and conventions, and clarifies the design by **grilling by default**: it settles only the decisions worth settling before code is written — those expensive to reverse, or where a competent implementer could reasonably go another way — asking one question at a time with a recommended answer, answering from the codebase instead of asking whenever it can, and reusing the product conclusions. In Claude Code a third rung sits between the codebase and you: a technical decision the code and conventions leave open, and that the agent cannot recommend with confidence, goes to the `dev:advisor` advisor first — an answer that settles it as a technical fact or the single sane path removes the question and is cited as evidence alongside `file:line`, while a genuine remaining trade-off still reaches you, with the advisor's view in the recommended answer. If the advisor is unavailable or leaves the point open, the question reaches you as usual. Approaches are compared only where a real fork exists. The direction is stated as short as the risk allows, at the altitude of boundaries and contracts rather than edits, and names what is left to the executor. Say "don't grill me" to switch to minimal questioning. It can also stress-test an existing plan or design document, producing revision notes instead of a new direction.

All three skills share a **start contract**, an internal completeness check rather than a questionnaire:

- **Direction and scope**: ask only about concrete unresolved ambiguities or tradeoffs; never ask whether the requirement is confirmed.
- **Stop after**: derive discussion, plan, or implementation from the user's request. Ask only if that endpoint is unclear.
- **Execution**: reuse the user's choice; otherwise use a host-supported default when execution is needed. Discussion and plan-only work can defer this choice.
- **Workspace**: inspect it and apply the isolation rules; report actual blockers instead of asking routine check questions.

The handoff retains each value and its basis (user decision, default, inspected fact, or not applicable). A later user instruction supersedes an older plan field. A specified executor that cannot run is reported as a blocker, never silently replaced. Optional executor choices list eligible executors directly, without an "others" follow-up menu.

Expected behavior for common requests:

| Request or condition | Result |
| --- | --- |
| "Implement this with opus and verify it." | Reuse the named model when supported; proceed without asking for requirement or execution confirmation. |
| "Think through how this could work." | Discuss and stop; do not ask for an executor or start writing. |
| "Write a plan first; I want to review it." | Write the plan and stop, leaving an unspecified executor deferred. |
| "Now execute that plan." | Update the older plan-only endpoint and execute, preserving existing choices. |
| A named executor is unavailable | Report the blocker and request a replacement if needed; do not silently choose another. |
| A material product decision remains open | Ask about that specific decision, then continue within the requested scope. |

### 2. Plan (`dev:write-plan`)

`dev:write-plan` writes one plan per requirement under `wiki/plans/` as an **outcome contract**: the requirement — carrying the product conclusions when the change is perceptible — the settled decisions with their tradeoffs, landmines, a scope boundary, validation commands, done criteria, stop conditions, and `Execution:`, `Stop after:`, and `Workspace:` fields carrying the resolved choices and their basis — leaving implementation design to the executor. It never edits source code and never re-asks settled decisions; minor decisions that surface during planning are made following the settled direction and recorded in the plan.

**Off the main worktree.** When the session is in the repository's main worktree — on any branch — `dev:write-plan` first cuts a branch `dev/YYYYMMDD-slug` and a worktree `.claude/worktrees/YYYYMMDD-slug` from the current `HEAD`, named after the requirement with the same id as the plan, and switches the session into it. The plan commit, the implementation commits, and the status updates all land on that branch; the main worktree keeps its branch untouched, and merging back is yours to request. A dirty tree stops the move rather than leaving uncommitted changes behind; the agent reports the pending changes. The planner installs nothing there — `dev:execute-plan` installs dependencies at preflight. Say "plan here" in your request, or start from an existing linked worktree, to skip the move.
...
| Mode | When | Notes |
| --- | --- | --- |
| Subagent (default) | The host has a subagent/task tool (e.g. Claude Code's `Agent`). | Dispatches the host's generic subagent with `model: opus` only where supported, otherwise the host default; runs inside the host's existing permission envelope, so no extra consent is needed. |
...
The default subagent is the host's generic one using `opus` where supported, otherwise the host default. To run a non-Claude model served through your API relay, name a model-pinned executor agent in your request — the plugin ships one executor per relay vendor, exposed as `dev:<vendor>-executor`, each pinning a full model ID in its frontmatter. The agent files under `agents/` are the source of truth for which model each one runs — read the ID there rather than trusting any list in the docs, since relay model IDs move. You can define more the same way in `.claude/agents/` or the user configuration's `agents/` directory. Dispatch pinned executors without a `model` override. Verify which model actually served the run (for example via relay-side logs); a listed model is not proof of actual routing.

**Executor discovery runs in hooks.** With Node.js 22 or later on `PATH`, the plugin's `SessionStart` command hook reads executor bindings, queries the relay's model list, and injects a concise availability snapshot. It runs on new sessions, resume, clear, and compaction, reusing fresh results rather than asking the orchestrator to issue requests. Optional executor selection consumes this snapshot: unavailable executors are excluded, and unverified executors remain explicitly labeled. A recorded user selection is retained; discovery never automatically switches executors.
